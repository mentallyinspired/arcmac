...
emacs --batch --quick --eval "(require 'ob-tangle)" \
  --eval '(org-babel-tangle-file "config/config.org")'
#+end_src
...
=user-emacs-directory= is a symlink into this git checkout, so
...
The config dir is a symlink into the repo; anything Emacs writes at
...
All servers are already on PATH from =modules/dev/*=: nixd, gopls,
rust-analyzer, pyright, zls, bash/typescript/yaml language servers.
...
Notmuch over the maildirs that =modules/mail.nix= maintains (mbsync +
msmtp + afew, accounts =danros= and =septus=). The elisp ships with
...
next sync (see =modules/mail.nix=). The sync command runs the full
