...
(defun nd/all-organization-roster ()
  "Return (BREADCRUMB . ID) entries for organizations and nested units."
  (with-current-buffer (find-file-noselect "~/org/ref/organizations.org")
    (org-with-wide-buffer
     (delq nil
           (org-map-entries
            (lambda ()
              (when-let ((id (org-id-get)))
                (cons (string-join (org-get-outline-path t t) " / ") id))))))))

(defun nd/organization-heading-for-id (id)
  "Return the heading text for ID in organizations.org."
  (with-current-buffer (find-file-noselect "~/org/ref/organizations.org")
    (org-with-wide-buffer
...
(defun nd/read-organizational-unit ()
  "Prompt for any organization or nested unit and return its ID link."
  (let* ((roster (nd/all-organization-roster))
         (choice (completing-read "Organizational unit: "
                                  (mapcar #'car roster) nil t)))
    (if (string-empty-p choice)
        ""
      (let ((id (cdr (assoc choice roster))))
        (format "[[id:%s][%s]]" id (nd/organization-heading-for-id id))))))

(defun nd/org-id-from-link (link)
  "Return the ID contained in an Org LINK string, or nil."
  (when (and link (string-match "\\[\\[id:\\([^]]+\\)\\]" link))
    (match-string 1 link)))

(defun nd/organization-subtree-ids (id)
  "Return ID and the IDs of every organizational unit nested below it."
  (with-current-buffer (find-file-noselect "~/org/ref/organizations.org")
    (org-with-wide-buffer
     (goto-char (or (org-find-entry-with-id id)
                    (user-error "Unknown organizational unit ID: %s" id)))
     (save-restriction
       (org-narrow-to-subtree)
       (delq nil (org-map-entries #'org-id-get))))))

(defun nd/list-contacts-by-org-unit ()
  "List contacts assigned to a selected unit or anything below it."
  (interactive)
  (let* ((roster (nd/all-organization-roster))
         (choice (completing-read "Organization or unit: "
                                  (mapcar #'car roster) nil t))
         (unit-ids (nd/organization-subtree-ids (cdr (assoc choice roster))))
         contacts)
    (with-current-buffer (find-file-noselect "~/org/ref/people.org")
      (org-with-wide-buffer
       (org-map-entries
        (lambda ()
          (when (member (nd/org-id-from-link (org-entry-get nil "ORG_UNIT"))
                        unit-ids)
            (push (cons (org-get-heading t t t t) (org-id-get)) contacts)))
        "LEVEL=1")))
    (with-current-buffer (get-buffer-create "*Contacts by organization*")
      (let ((inhibit-read-only t))
        (erase-buffer)
        (insert "#+title: Contacts — " choice "\n\n")
        (dolist (contact (sort contacts (lambda (a b)
                                          (string-collate-lessp (car a) (car b)))))
          (insert (format "- [[id:%s][%s]]\n" (cdr contact) (car contact))))
        (org-mode)
        (goto-char (point-min))
        (view-mode 1))
...
(defun nd/contact-find-location ()
  ...
  (setq nd/capture-contact-name (read-string "Name: "))
...
(with-eval-after-load 'org
  (setq org-capture-templates
        `(("i" "Inbox" entry (file ,nd/inbox-file)
          ...
          ("c" "Contact" entry
           (file+function "~/org/ref/people.org" nd/contact-find-location)
           "* %(progn nd/capture-contact-name)\n:PROPERTIES:\n:ID:         %(org-id-new)\n:context:    %^{Context||Sigtuna|private|veltric|nordic}\n:org_unit:   %(nd/read-organizational-unit)\n:position:   %^{Position}\n:email:      %^{Email}\n:tel_mobile: %^{Mobile}\n:END:\n%?"
           :empty-lines 1
           :insert-here t)

          ("o" "Organization" entry
           (file+function "~/org/ref/organizations.org"
                          nd/organization-find-location)
           "* %(progn nd/capture-organization-name)\n:PROPERTIES:\n:ID:        %(org-id-new)\n:unit_type: %^{Unit type||municipality|company|authority|association}\n:website:   %^{Website}\n:email:     %^{Email}\n:tel:       %^{Telephone}\n:END:\n%?"
           :empty-lines 1
           :insert-here t)
