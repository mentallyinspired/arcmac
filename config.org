...
emacs-overlay — see =emacs-package.nix=.
...
a one-line justification in =emacs-package.nix= — if a line
...
=C-v= and =C-S-v= paste from the clipboard in minibuffer prompts,
including URLs, filenames and completion fields. =C-y= also remains available.
...
The prompt names the file and warns about unsaved edits. It respects Emacs'
=delete-by-moving-to-trash= setting. Use the source file when working in an
indirect person, project or capture view.
...
=emacs-package.nix=), so UI and index can never version-drift.
...
Delete = tag =+deleted -inbox=; the notmuch pre-new hook moves tagged mail
to Trash on the next sync (see =mail.nix=). Emacs, =mail-sync= and the
five-minute timer share one fetch/index/push command. Concurrent manual
requests report that sync is already running; a send queues a sync behind
any current run so its Sent copy is included. Output is kept in
=*notmuch-sync*=; timer output is in =journalctl --user -u mbsync=.
...
Agenda jumps and task links expand the path to the target and show sibling
headings along that path. The task's own notes are visible and its drawers
stay folded, with the cursor on the target. File startup settings such as
=#+startup: overview= still control how a file opens normally.
...
=org-tag-persistent-alist= supplies the choices for tag editing and the
work-log capture prompt.
...
Organization selectors match text anywhere in the full path or aliases,
so a company-name prefix cannot hide a matching nested unit.
...
Capture opens in a temporary panel across the bottom third of the frame,
keeping existing work windows visible above it. =C-c C-c= saves the capture;
=C-c C-k= cancels it. Both close the panel and restore the previous layout.

Inbox capture opens directly for writing. Contact capture asks only for
a name, with a confirmation for an existing name. Add other contact
details with =SPC n c e= from the person view, or =e= in the directory.
Each field starts with its current value and can be changed independently.

New note captures require a nonempty title. Existing filenames, including
names reserved by unsaved buffers, get =-2=, =-3= and so on; a new capture
never appends a second file header to an existing note.

Meeting capture asks only for a title and starts under Notes. The ID,
timestamp and =meeting:unprocessed= tags are automatic. Use =SPC m E=
to add attendees, organisation, project links or tags during preparation
or review. Clear =unprocessed= with =SPC m q= once decisions are clear
and actions have been assigned or filed; editing details does not clear it.
...
into today's entry via =nd/org-journal-find-location=. Opening the journal
or starting an entry uses the selected window, including after =SPC w n=.
...
Automatic saving applies only to local Org files inside =org-directory=.
Code and Org files elsewhere keep ordinary explicit saving. Indirect
person and project views share the saving policy of their source file.
...
The portfolio (=SPC n p=) lists each project's NEXT tasks underneath,
indented and in source order. This includes future-scheduled tasks and
tasks in held or finished projects. Archived and commented tasks stay
hidden; nested projects own their tasks separately. Task rows retain
normal agenda navigation and editing. Refresh with =g r= to rebuild the list.
...
=SPC n /= searches =org-directory= with ripgrep, installed by the Nix
module. =SPC n R= is optional Recoll document search. Install Recoll
(=home.packages = [ pkgs.recoll ];=), choose indexed folders in its
=recoll.conf=, and run =recollindex= before using it. =RECOLL_CONFDIR=
is respected for a custom configuration. The command checks availability
and index access and shows setup guidance if either is missing.
...
=SPC m U= updates its =Nuläge= and =Återuppta= summary, prefilling existing
text and refreshing =Uppdaterad= when either changes. Older projects get
missing fields automatically; child tasks and detailed plans are preserved.

=SPC m w= asks who you are waiting for and when to follow up, then sets
the task to WAIT. Choose a directory contact or type an unregistered name.
The contact is stored as an ID link in =WAITING_FOR=; the date uses Org's
normal =SCHEDULED= field. Cancelling either prompt leaves the task unchanged.
Waiting and weekly review views display the owner and follow-up date,
including explicit reminders for missing information on older tasks.

=SPC m E= edits one meeting detail from anywhere inside the meeting, or
from its agenda entry. Select Attendees, Organisation, Projects or Tags;
current selections are prefilled and can be removed or extended.
